...
class KeywordsStoppingCriteria(StoppingCriteria):
    ...
    def __call__(self, output_ids: torch.LongTensor, scores: torch.FloatTensor, **kwargs) -> bool:
        ...
        if self.start_len is None:
            self.start_len = self.input_ids.shape[1]
        # 遍历批次中的每个样本，检查是否生成结束
        for i in range(output_ids.size(0)):  # 遍历每个样本
            if self.stopped[i]:  # 如果该样本已经停止，继续生成 pad_token_id
                output_ids[i, -1] = self.pad_token_id
            else:
                # 检查最新生成的 token 是否是关键词的 token ID
                if output_ids[i, -1].item() in self.keyword_ids:
                    self.stopped[i] = True  # 该样本停止生成，开始填充
                else:
                    # 解码生成的输出，检查是否包含关键词
                    outputs = self.tokenizer.batch_decode(output_ids[i, self.start_len:], skip_special_tokens=True)
                    for keyword in self.keywords:
                        if keyword in outputs[0]:  # 包含关键词
                            self.stopped[i] = True 
